 ...
 *
 * <p><strong>Estratégia DB-first com fallback para memória:</strong></p>
 * <ol>
 *   <li>Tenta buscar ECOs/puzzles no banco de dados (partidas salvas e analisadas).</li>
 ...
 *   <li>{@code GET  /api/puzzles?eco=XXX}   — puzzles do ECO informado (DB-first)</li>
 *   <li>{@code POST /api/puzzles/evaluate}  — avalia o lance do usuário com Stockfish</li>
 * </ul>
 */
@Controller
public class PuzzleController {
    ...
    public PuzzleController(PuzzleDbService     puzzleDbService,
     ...
     *
     * <p><strong>Fonte 1 — Banco de dados:</strong> partidas persistidas com
     * {@code lance.blunder = true} e {@code lance.analisado = true}.</p>
     *
     * <p><strong>Fonte 2 — Memória (fallback):</strong> partidas carregadas via
     * PGN na sessão atual, quando o banco não possui dados analisados.</p>
     */
     ...
     *
     * <p>Segue a mesma estratégia DB-first com fallback para memória.</p>
     */
    ...
    public ResponseEntity<?> getPuzzles(@RequestParam String eco) {
        ...
        return ResponseEntity.badRequest().body(Map.of(
                "ok",      false,
                "message", "Nenhum puzzle encontrado para " + eco +
                           ". Carregue um PGN na aba principal e analise as partidas."));
    }
     ...
     *
     * <p>Body esperado: {@code { fen, from, to, promotion, evalBefore, bestMove, depth }}</p>
     */
    ...
    public ResponseEntity<?> evaluate(@RequestBody EvaluateRequest req) {
        if (!stockfish.isReady()) {
            return ResponseEntity.badRequest().body(Map.of("ok", false,
                    "message", "Stockfish não conectado. Configure-o na aba principal."));
        }

        // 1. Aplica o lance do usuário (valida legalidade)
        MoveResponse mv = boardService.applyMove(
                new MoveRequest(req.fen(), req.from(), req.to(), req.promotion()));
        if (!mv.ok()) {
            return ResponseEntity.badRequest()
                    .body(Map.of("ok", false, "message", mv.message()));
        }

        // 2. Analisa a nova posição com o Stockfish
        int depth = req.depth() == null || req.depth() <= 0
                ? analysisService.getAnalysisDepth() : req.depth();
        StockfishResult sf;
        try {
        ...
        boolean whiteToMove = req.fen() != null
                && req.fen().split("\\s+").length > 1
        ...
        Map<String, Object> classification = puzzleService.classifyMove(
                req.evalBefore() == null ? 0.0 : req.evalBefore(),
                sf.mateIn() != null ? null : sf.eval(),
                sf.mateIn(), whiteToMove, isBest);
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "playedUci",         userUci,
                "check",             mv.check(),
                "checkmate",         mv.checkmate(),
                "stockfishBestMove", sf.bestMove() == null ? "" : sf.bestMove(),
                "stockfishPv",       sf.pv() == null ? List.of() : sf.pv().stream().limit(5).toList(),
